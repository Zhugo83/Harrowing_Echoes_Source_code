...
class ChartingState extends MusicBeatState
{
	...
	var noteType:Int = 0;
	var noteTypeList:Array<String> = ['normal', 'warning'];
	...
	**/
	...
	function addNoteUI():Void
	{
		...
		//for note type shit now:

		tab_group_note_type = new FlxUI(null, UI_box);
		tab_group_note_type.name = 'Type';

		writingNotesText = new FlxUIText(20,100, 0, "");
		writingNotesText.setFormat("Arial",20,FlxColor.WHITE,FlxTextAlign.LEFT,FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);

		var noteTypeLabel = new FlxText(74,10,'Note Type Dropdown');

		var noteTypeDrop = new FlxUIDropDownMenu(74, 25, FlxUIDropDownMenu.makeStrIdLabelArray(noteTypeList, true), function(type:String)
		{
			noteType = Std.parseInt(type);
		});
		noteTypeDrop.selectedLabel = noteTypeList[noteType];

		tab_group_note_type.add(noteTypeLabel);
		tab_group_note_type.add(noteTypeDrop);

		UI_box.addGroup(tab_group_note_type);
	...
	override function update(elapsed:Float)
	{
		...
		for (p in 0...pressArray.length)
		{
			...
			if (i && !delete)
			{
				addNote(new Note(Conductor.songPosition, p, null, false, _song.player1, noteTypeList[noteType]));
			}
		}
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var note:Note = new Note(daStrumTime, daNoteInfo % 4, null, false, 'bf', daType);
	...
	private function addNote(?n:Note):Void
	{
		...
		var noteSus = 0;
		var noteSkin:String = _song.player1;
		var noteType:String = noteTypeList[noteType];

		if (n != null)
			_song.notes[curSection].sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.noteSkin, n.noteType]);
		else
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, noteSkin, noteType]);
